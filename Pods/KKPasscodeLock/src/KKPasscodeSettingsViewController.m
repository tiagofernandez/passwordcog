...
- (void)eraseDataSwitchChanged:(id)sender 
{
	if (_eraseDataSwitch.on) {
		NSString* title = [NSString stringWithFormat:@"All passwords will be erased after %d failed passcode attempts.", [[KKPasscodeLock sharedLock] attemptsAllowed]];
...
{
	if (section == 2) {
		return [NSString stringWithFormat:@"Erase all data in the app after %d failed passcode attempts.", [[KKPasscodeLock sharedLock] attemptsAllowed]];;
	} else {
		return @"";
	}
}
