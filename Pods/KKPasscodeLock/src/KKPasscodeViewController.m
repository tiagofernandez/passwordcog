...
- (void)loadView
{
	...
	_enterPasscodeTableView.backgroundColor = [UIColor underPageBackgroundColor];
	...
	_setPasscodeTableView.backgroundColor = [UIColor underPageBackgroundColor];
	...
	_confirmPasscodeTableView.backgroundColor = [UIColor underPageBackgroundColor];
...
- (void)viewWillAppear:(BOOL)animated 
{
  ...
  if (_mode == KKPasscodeModeSet) {
    self.navigationItem.title = @"Turn Passcode On";
    ...
    self.navigationItem.title = @"Turn Passcode Off";
    self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel
                                                                                           target:self
                                                                                           action:@selector(cancelButtonPressed:)] autorelease];

  } else {
    self.navigationItem.title = @"Passwordcog";
  }
...
- (void)incrementFailedAttemptsLabel
{
  AudioServicesPlaySystemSound(kSystemSoundID_Vibrate);
	...
	if (_failedAttemptsCount == 1) {
		_failedAttemptsLabel.text = @"1 Failed Attempt";
	} else {
    ...
    // https://github.com/aporat/KKPasscodeLock/issues/3
